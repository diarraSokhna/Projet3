...
public class SiteDaoImpl implements SiteDao {
	...
	public Site creer(Site site) throws DaoException {
		...
		try {
			...
			preparedStatement = initialisationRequetePreparee(connexion, SQL_INSERT, true, site.getNomsite(),
					site.getPays().getIdpays(), site.getImage(), site.getClassement().getId_class(),
	...
	public List<Site> listerParClassement(long idclassement) throws DaoException {
		...
		try {
			connection = daoFactory.getConnection();
			preparedStatement = initialisationRequetePreparee(connection, SQL_SELECT_PAR_CLASSEMENT, false,
					idclassement);
			resultSet = preparedStatement.executeQuery();
			while (resultSet.next()) {
				sites.add(map(resultSet));
