...
public class ReservationDaoImpl implements ReservationDao {
	...
	public void creer(Reservation reservation) throws DaoException {
		...
		try {
			...
			preparedStatement = initialisationRequetePreparee(connexion, SQL_INSERT, true, reservation.getDate_resa(),
					reservation.getTopo().getIdtopo(), reservation.getUtilisateur().getIduser());
			...
			if (valeursAutoGenerees.next()) {
				...
				throw new DaoException(
						"Échec de la création de la réservation en base, aucun ID auto-généré retourné.");
			}
	...
	public void supprimer(Reservation reservation) throws DaoException {
		...
		try {
			...
			preparedStatement = initialisationRequetePreparee(connexion, SQL_DELETE_PAR_ID, true,
					reservation.getId_resa());
			int statut = preparedStatement.executeUpdate();
			if (statut == 0) {
				throw new DaoException(
						"Échec de la suppression de la reservation, aucune ligne supprimée de la table.");
			} else {
